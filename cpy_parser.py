...
class Parser:
    ...
    def def_main_function(self):
        ...
        if token.recognized_string == "def":
            ...
            if token.family == "identifier" and re.search("^main_[a-zA-Z0-9]+", token.recognized_string): # Starts with "main_" and has at least one letter or digit after
                ...
                self.error("Expected identifier that starts with 'main_' in def_main_function") # Maybe a better error
        else:
    ...
    def statements(self):
        self.statement()
        while token.recognized_string == "if" or token.recognized_string == "while" or token.family == "identifier" or token.recognized_string == "print" or token.recognized_string == "return":
            self.statement()
